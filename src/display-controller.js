  ...
  for (const project of projects) {
    ...
    if (project.todoArray.length != 0) {
      for (const todo of project.todoArray) {
        ...
        if (todo.complete)
          completeBtn.style.cssText = "background-color: green";
        ...
        editBtn.addEventListener("click", () => {
          showEditor(
            projects,
            editBtn.getAttribute("project"),
            editBtn.getAttribute("index"),
          );
        });
...
function showEditor(projects, projectName, index) {
  ...
  for (const project of projects) {
    if (project.title === projectName) {
      ...
      if (dueDateFormat) {
        editDueDate.value = dueDateFormat
          .toISOString()
          .split("T")[0]
          .slice(0, 10);
      }
...
updateBtn.addEventListener("click", (e) => {
  ...
  if (editDueDate.valueAsDate) {
    editDueDateFormat = new Date(
      editDueDate.valueAsDate.toISOString().replace(".000Z", ""),
    );
  }
